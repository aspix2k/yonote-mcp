...
Укажите ваш API-токен в `.env`:

```
YONOTE_API_TOKEN=ваш-токен
YONOTE_API_BASE_URL=https://app.yonote.ru/api
```
...
```json
{
  "mcpServers": {
    "yonote": {
      "command": "node",
      "args": ["/абсолютный/путь/к/yonote-mcp/dist/index.js"],
      "env": {
        "YONOTE_API_TOKEN": "ваш-токен"
      }
    }
...
```bash
claude mcp add yonote -e YONOTE_API_TOKEN=ваш-токен -- node /абсолютный/путь/к/yonote-mcp/dist/index.js
```
...
- **command**: `node`
- **args**: `["/путь/к/yonote-mcp/dist/index.js"]`
- **env**: `{ "YONOTE_API_TOKEN": "ваш-токен" }`
...
|---|---|---|---|
| `YONOTE_API_TOKEN` | Да | — | API-токен Yonote |
| `YONOTE_API_BASE_URL` | Нет | `https://app.yonote.ru/api` | Базовый URL API |
